...
class T5Attention(Module):
    ...
    def forward(
        ...
        # orig_shape is only set when tensor_parallel.factor > 1 (unsqueeze path).
        # For tp=1 there is no unsqueeze, so skip the reshape entirely and return
        # dense_out directly — it already has the correct (batch, seq_len, hidden)
        # shape from o_proj.
        if self.parallel_config.tensor_parallel.factor > 1:
