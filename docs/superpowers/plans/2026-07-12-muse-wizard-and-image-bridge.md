...
5. **Content stretches full-width** (user feedback) — Discover/Open/composer content expands to the entire window width even when there's little content, which reads as unbalanced. Fix: constrain the primary content column to a comfortable reading/gallery max-width (≈ 900–1000px) and center it (`set_halign(Gtk.Align.CENTER)` on the content box with a `set_size_request`/max-width wrapper, or an `Adw.Clamp`-style cap if available — otherwise a plain centered box with a max width), so panels don't sprawl across a wide screen. Apply consistently to DiscoverView, OpenView, and RemixView content.
6. **Each step's produced content should be genuinely visible** (user feedback) — the Open view currently shows only a small thumbnail per step. Fix: give each step's *real produced artifact* prominence — a larger preview of the actual content (image rendered larger; text/caption/prompt shown as readable inline text, not just an icon; video/gif a real frame/looping preview where feasible). The point is to SEE what each step made, not a token. Keep it honest for pending/failed steps (the tidy intent-icon tile from #4). This may mean a taller-but-content-rich row rather than the compact row of #2 for steps that HAVE content — reconcile: compact when empty, expansive (showing real content) when the step produced something.

- [ ] **Step 1: tests** — assert the OpenView step row now contains a single combined intent label (one label whose text == `f"{verb} {noun}"`) rather than separate verb/noun widgets; assert `_build_thumb_frame(None, ...)` for a step renders the intent icon (pass the intent through), not the bare 🖼 fallback; assert the DiscoverView/OpenView/RemixView content column has a bounded max width (a wrapper with `get_halign() == Gtk.Align.CENTER` and a finite width request, not `hexpand` sprawl); assert a step WITH a text artifact renders its text inline (a readable label containing the artifact text), and a step with an image artifact requests a preview larger than the old 150×92 thumb. (Structural assertions via the widget tree under xvfb.) → fail.
- [ ] **Step 2: Run to verify fail.**
- [ ] **Step 3: Implement** all six fixes. Keep changes CSS/layout-local; do not alter data flow. For #6, prefer reusing the app's existing media-rendering helpers (how the main gallery renders an image/text/video artifact) rather than inventing new ones. Verify visually is out of scope for CI — assert structure.
