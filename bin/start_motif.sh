#!/usr/bin/env bash
# start_motif.sh — Start the Motif-Image-6B-Preview server on P300X2.
#
# Hardware note:
#   4× Wormhole p300c PCIe cards = 2 logical p300 boards (L/R dies) = DeviceTypes.P300X2
#   Motif-Image-6B-Preview uses a 2×2 device mesh on P300X2.
#
# Status: COMPLETE (validated on P300X2 in tt-inference-server v0.17.0).
#
# The API is synchronous: POST /v1/images/generations returns a base64 JPEG.
# Note: uses Docker image 0.9.0-c180ef7 (different from FLUX/Wan2.2).
#
# Usage:
#   ./start_motif.sh          # start server and tail its log
#   ./start_motif.sh --stop   # stop the running server container
#   ./start_motif.sh --gui    # start without interactive prompts or tail (for GUI use)
#   ./start_motif.sh --help   # show this help
...
HF_REPO="Motif-Technologies/Motif-Image-6B-Preview"
HF_CACHE_DIR="$HF_CACHE/hub/models--Motif-Technologies--Motif-Image-6B-Preview"
...
if [[ ! -d "$HF_CACHE_DIR" ]]; then
    echo "WARNING: Motif-Image-6B-Preview weights not found at $HF_CACHE_DIR"
    echo "         Pre-download with:"
    echo "           huggingface-cli download ${HF_REPO}"
    echo "         (~15–25 GB estimated)"
...
echo ""
echo "Tip: the server prints 'Application startup complete' when ready (~5–15 min first run)."
echo ""
