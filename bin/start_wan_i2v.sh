#!/usr/bin/env bash
# start_wan_i2v.sh — Start the Wan2.2-I2V-A14B-Diffusers (image-to-video) server on P300x2 (QB2).
#
...
#   - P300X2 mesh: (2,2) — 2 P300 cards × 2 chips each (2-way SP × 2-way TP)
#
# The workflow process writes a log file to:
#   tt-inference-server/workflow_logs/docker_server/media_<timestamp>_Wan2.2-I2V-A14B-Diffusers_p300x2_server.log
# This script waits for that file to appear then tails it live.
#
# Usage:
#   ./start_wan_i2v.sh            # start server and tail its log
#   ./start_wan_i2v.sh --stop     # stop the running server container
#   ./start_wan_i2v.sh --gui      # start without interactive prompts or tail (for GUI use)
#   ./start_wan_i2v.sh --help     # show this help
...
LOG_GLOB="media_*_Wan2.2-I2V-A14B-Diffusers_p300x2_server.log"
...
if [[ ! -d "$HF_CACHE/hub/models--Wan-AI--Wan2.2-I2V-A14B-Diffusers" ]]; then
...
echo "Starting Wan2.2-I2V-A14B-Diffusers on P300x2 (QB2)…"
echo "  Repo:      $REPO_DIR"
echo "  Image:     $DOCKER_IMAGE"
echo "  HF cache:  $HF_CACHE  (bind-mounted read-only)"
echo "  Port:      8000"
echo "  Mesh:      (2,2) — P300X2"
echo ""
...
echo ""
echo "Tip: the server prints 'Application startup complete' when ready (~5-10 min on P300x2)."
echo ""
